...
// Crée un compte, le marque comme vérifié directement en base (on ne teste
// pas le flux email ici, voir tests/auth.test.js pour ça — la connexion est
// désormais bloquée tant que l'email n'est pas confirmé), puis le connecte.
async function creerUtilisateurConnecte(app, overrides = {}) {
  ...
  await User.findOneAndUpdate({ email: data.email.toLowerCase() }, { emailVerifie: true });
