...
export default function Clients() {
  ...
  const load = () => {
    setLoading(true);
    getClients({ page, limit: 24, q: search || undefined })
      .then((res) => {
  ...
  // Recherche : on repart en page 1 et on interroge le serveur après une
  // courte pause, pour ne pas envoyer une requête à chaque frappe.
  useEffect(() => {
    const t = setTimeout(() => {
      if (page !== 1) setPage(1);
      else load();
  ...
  return (
    <div>
      <PageHeader
        title="Clients"
        subtitle={`${total} client(s) au total`}
        icon={<Users size={20} />}
        actions={
