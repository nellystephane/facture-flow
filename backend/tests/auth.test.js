...
describe('POST /api/auth/login', () => {
  ...
  it('connecte normalement une fois l’email confirmé', async () => {
    const spy = jest.spyOn(email, 'sendVerificationCode').mockResolvedValue(true);
    await request(app).post('/api/auth/register').send(utilisateurValide);
    const code = spy.mock.calls[0][0].code;
    await request(app).post('/api/auth/verifier-email').send({ email: utilisateurValide.email, code });
  ...
  it('refuse un mauvais mot de passe (même message générique, avant de révéler quoi que ce soit sur l’email)', async () => {
    jest.spyOn(email, 'sendVerificationCode').mockResolvedValue(true);
    await request(app).post('/api/auth/register').send(utilisateurValide);
    const res = await request(app)
      .post('/api/auth/login')
    ...
    expect(res.status).toBe(400);
    expect(res.body.code).not.toBe('EMAIL_NON_VERIFIE');
  });
});
