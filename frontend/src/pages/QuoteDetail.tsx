...
export default function QuoteDetail() {
  ...
  const partagerWhatsApp = () => {
    ...
    const clientRattache = typeof quote.client === 'object' ? quote.client : null;
    const prenom = clientRattache?.nom ? clientRattache.nom.split(' ')[0] : '';
    const lien = publicAppUrl(`devis/${quote.publicToken}`);
    const validite = quote.dateExpiration ? ` (valable jusqu'au ${formatDate(quote.dateExpiration)})` : '';
    const message = [
      `Bonjour ${prenom},`.trim(),
  ...
  if (!quote) return <div className="text-center text-gray-500 dark:text-gray-400 py-20">Devis introuvable.</div>;
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6 animate-fade-up">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl md:text-3xl font-extrabold text-[#0a0a0c] dark:text-white">{quote.numero}</h1>
              <span className={badgeClass(quote.statut)}>{QUOTE_STATUT_LABEL[quote.statut]}</span>
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400">Émis le {formatDate(quote.dateEmission)}</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {quote.statut !== 'accepte' ? (
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1" title="Ce devis a déjà été accepté et a généré une facture : il ne peut plus être modifié ni supprimé.">
      ...
      {quote.demandeInfo?.message && (
        <div className="glass-card p-5 mb-6 animate-fade-up border border-amber-200" style={{ background: 'rgba(245,158,11,0.06)' }}>
          <p className="text-sm font-bold text-[#0a0a0c] dark:text-white mb-1 flex items-center gap-2">
            <MessageCircleQuestion size={16} className="text-amber-600" /> Votre client a demandé des précisions
          </p>
          <p className="text-sm text-gray-600 dark:text-gray-400 italic">« {quote.demandeInfo.message} »</p>
          {quote.demandeInfo.date && (
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Reçu le {formatDate(quote.demandeInfo.date)}</p>
          )}
        </div>
      )}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-card p-6 animate-fade-up">
            <div className="grid sm:grid-cols-2 gap-6">
              <div>
                <p className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase mb-2">Adressé à</p>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Valable jusqu'au</span><span className="font-medium">{formatDate(quote.dateExpiration)}</span></div>
                  {quote.objet && <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Objet</span><span className="font-medium text-right">{quote.objet}</span></div>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6 animate-fade-up">
            ...
            {quote.notes && (
              <div className="mt-4 p-3 rounded-xl bg-gray-50 dark:bg-white/5 text-sm text-gray-600 dark:text-gray-400">
                <p className="font-semibold text-gray-700 dark:text-gray-300 mb-1">Notes</p>{quote.notes}
              </div>
            )}
          </div>
          ...
          {quote.publicToken && quote.statut !== 'accepte' && quote.statut !== 'refuse' && (
            <div className="glass-card p-6 animate-fade-up">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Inclus automatiquement dans l'email du devis — vous pouvez aussi le copier ou l'envoyer directement par WhatsApp ci-dessus.</p>
              <div className="flex flex-wrap gap-2">
                <input readOnly className="field flex-1 min-w-[220px] text-xs text-gray-500 dark:text-gray-400" value={publicAppUrl(`devis/${quote.publicToken}`)} onClick={(e) => (e.target as HTMLInputElement).select()} />
        ...
        <div className="space-y-6">
          <div className="glass-card p-6 animate-fade-up sticky top-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Sous-total</span><span className="font-medium">{formatFCFA(quote.items.reduce((s, i) => s + i.quantite * i.prixUnitaire, 0))}</span></div>
              {quote.remise ? <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Remise</span><span className="font-medium text-[#d9524d]">- {formatFCFA(quote.remise)}</span></div> : null}
              <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">TVA ({quote.tva}%)</span><span className="font-medium">{formatFCFA(ttc / (1 + quote.tva / 100) * quote.tva / 100)}</span></div>
      ...
      <PdfPreviewModal open={preview} onClose={() => setPreview(false)} url={quotePdfUrl(id!)} title={`Aperçu de ${quote?.numero || 'du devis'}`} filename={`Devis-${quote?.numero || id}.pdf`} />
