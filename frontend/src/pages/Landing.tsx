    ...
    <div className="app-bg">
      ...
      <div className="relative z-10">
        ...
        <section id="features" className="px-4 sm:px-6 lg:px-12 py-20 max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              { icon: Smartphone, title: 'Mobile Money', desc: 'Encaissez via MTN Money et Moov Money. Suivez chaque paiement en temps réel.' },
        ...
        <section id="pricing" className="px-4 sm:px-6 lg:px-12 py-20 max-w-7xl mx-auto">
          ...
          {plansLoading ? (
            <div className="grid md:grid-cols-3 gap-6">{[0, 1, 2].map((i) => <div key={i} className="h-96 skeleton" />)}</div>
          ) : (
            <div className="grid md:grid-cols-3 gap-6 items-stretch">
              {plans.map((plan, i) => {
                const optionAn = plan.options?.find((o) => o.duree === '1an');
                const prixAffiche = plan.id === 'gratuit' ? '0' : optionAn ? new Intl.NumberFormat('fr-FR').format(Math.round(optionAn.prix / 12)) : '—';
                const periode = plan.id === 'gratuit' ? '/mois' : 'FCFA/mois';
                return (
                  <div key={plan.id}
                    className={`glass-card p-7 flex flex-col animate-fade-up relative ${plan.recommande ? 'ring-2 ring-[#d9524d]' : ''}`}
                    ...
                    <h3 className="text-xl font-bold text-[#0a0a0c]">{plan.nom}</h3>
                    <p className="text-xs text-gray-400 mt-0.5">{plan.accroche}</p>
                    <div className="flex items-baseline gap-1 mt-3 mb-1">
                      <span className="text-4xl font-extrabold text-[#0a0a0c]">{prixAffiche}</span>
                      <span className="text-sm text-gray-500">{periode}</span>
                    </div>
                    {plan.id !== 'gratuit' && <p className="text-xs text-gray-400 mb-1">Facturé {optionAn ? new Intl.NumberFormat('fr-FR').format(optionAn.prix) : ''} FCFA à l'année</p>}
                    <ul className="space-y-3 my-6 flex-1">
                      {plan.avantages.map((f, j) => (
                        <li key={j} className="flex items-start gap-2.5 text-sm text-gray-700">
                          <Check size={18} className="text-green-600 shrink-0 mt-0.5" /> {f}
                        </li>
                      ))}
                    </ul>
                    <Link to="/register" className={plan.recommande ? 'btn-primary justify-center' : 'btn-dark justify-center'}>
                      {plan.id === 'gratuit' ? 'Commencer' : `Choisir ${plan.nom}`}
                    </Link>
                  </div>
                );
              })}
        ...
        <footer className="px-4 sm:px-6 lg:px-12 py-10 max-w-7xl mx-auto">
          <div className="glass rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-5 text-xs text-gray-500">
              <Link to="/confidentialite" className="hover:text-[#d9524d]">Confidentialité</Link>
              <Link to="/cgu" className="hover:text-[#d9524d]">CGU</Link>
              <Link to="/mentions-legales" className="hover:text-[#d9524d]">Mentions légales</Link>
            </div>
          </div>
        </footer>
      </div>
