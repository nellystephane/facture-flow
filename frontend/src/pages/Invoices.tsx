...
import { FileText, Plus, Search, Eye, Download, Filter, FileSpreadsheet, Loader2 } from 'lucide-react';
import { getInvoices, invoicePdfUrl, invoicesExportUrl } from '../api/invoices';
...
export default function Invoices() {
  ...
  const load = () => {
    setLoading(true);
    getInvoices({ statut: statut || undefined, q: search || undefined, page })
      .then((res) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast("Échec de l'export. Réessayez.", 'error');
    } finally {
      setExporting(false);
    }
  };
    ...
    <div>
        ...
        actions={
          <div className="flex gap-2">
            <button
              ...
              title={permissions?.peutExporterComptabilite ? 'Exporter en CSV' : 'Export comptable réservé aux plans Pro et Business'}
              className="btn-ghost text-sm disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {exporting ? <Loader2 size={16} className="animate-spin" /> : <FileSpreadsheet size={16} />} Export
            </button>
            <Link to="/app/invoices/new" className="btn-primary text-sm"><Plus size={18} /> Nouvelle facture</Link>
          </div>
        }
      />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={search || statut ? 'Essayez d\'autres critères.' : 'Créez votre première facture en quelques secondes.'}
          action={!search && !statut && <Link to="/app/invoices/new" className="btn-primary"><Plus size={18} /> Créer une facture</Link>}
        />
      ) : (
